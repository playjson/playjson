...
type ExtKV = { key: string; value: string };
  ...
  .action(async (file: string, opts: {
    ...
    try {
      ...
      if (ext === '.json') {
        ...
        const anyJsonnet = jsonnet as unknown as {
          evaluateFile?: (p: string) => Promise<string>;
        };
...
function parseKV(s: string): ExtKV {
  const i = s.indexOf('=');
  if (i < 0) throw new Error(`잘못된 형식: '${s}' (key=value 형태 필요)`);
  const key = s.slice(0, i).trim();
  const value = s.slice(i + 1);
  if (!key) throw new Error(`잘못된 키: '${s}'`);
  return { key, value };
}
